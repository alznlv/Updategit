...
class SyncWithGitServer(object):
    ...
    def createXmlFileNodeByName(self,nodenamevalue,version=1.0):
        files=self._xml_dom.createElement("file")
        self._xml_root.appendChild(files)
    ...
    def getGitPullOutput(self):
        ...
        while p.poll() is None:
            ...
            if out.find('Already up-to-date.')>=0:
                ...
                if deletefinename.find(self._pathPrefix)>=0:
...
#                 print pre+newname
                ...
                outlist=out.split('|')
                if len(outlist)>1 and outlist[0].find(self._pathPrefix)>=0 and outlist[0].find('=>')==-1:
                    self._file_update_list.append(outlist[0].strip())
                    print self._file_update_list
        if p.poll() is None:
            p.kill()    
        print p.returncode
    def writeXmlToFile(self,xmlpath=_xml_file_path):
        try:
            f = open(xmlpath, 'w')
...
#    sync.writeXmlToFile('new.xml')
    sync.getGitPullOutput()
    print '------------------create file -----------------'
    print sync._file_create_list
    print '------------------delete file-------------------'
    print sync._file_delete_list
    print '------------------rename file-------------------'
    print sync._file_rename_dict
    print '------------------update file-------------------'
    print sync._file_update_list
